...
async function forumLogin(email, password) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 401 || res.status === 403) {
      message = 'Invalid email or password. Make sure you are using your PNKSOUNDS credentials.';
    }
...
/**
 * Verify the JWT token and fetch the full user object (including isAdmin/isStaff).
 * Returns { user } or null if the token is invalid.
 */
async function verifyToken() {
  const session = restoreSession();
  if (!session) return null;
  try {
    const res = await fetch(`${FORUM_API_URL}/api/auth/verify`, {
      method: 'POST',
      headers: {
    ...
    const data = await res.json();
    if (data && data.user) {
      // Update the cached session with the fresh user object
      cachedSession = { ...cachedSession, user: data.user };
      return data.user;
    }
    return null;
  } catch {
