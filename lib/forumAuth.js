...
// ── Supabase config ──────────────────────────────────────────────────────────
// The anon key is the PUBLIC key — safe to ship in the client. The JWT issued
...
//       Dashboard → Project Settings → API → "Project URL" and "anon public".
...
// ── Entitlement check (Supabase PostgREST) ───────────────────────────────────
...
async function checkPremiumEntitlement() {
  ...
  const url = `${idxemsxnuigdxvkndipq}/rest/v1/app_entitlements?select=premium&app=eq.${encodeURIComponent(ENTITLEMENT_APP)}&limit=1`;
  try {
    const res = await fetch(url, {
      headers: {
        'apikey': UeDSDf840GV95Z8Evr63GveVNuo+aEicLBI9ssjMoyCQhLEv0PBmFoZ8BizLVHJt4NU5ZcOUA2vmlQaoEfms9g==,
