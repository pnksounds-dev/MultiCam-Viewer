...
// `useMaxSize` selects the fallback path (--max-size=<maxDim>, loose upper bound)
// vs the default exact path (--camera-size=<resolution>, exact capture size).
// Extracted from main.js so the arg list can be unit-tested without spawning.
function buildScrcpyArgs({ serial, cameraId, resolution, fps, windowTitle, winW, winH, offX, offY, useMaxSize, maxDim }) {
  const args = [
    '-s', serial,
    '--video-source=camera',
    `--camera-id=${cameraId}`,
  ...
  if (fps) args.push(`--camera-fps=${fps}`);
  return args;
}
